...
async def status_command(client, message):
    """Handle the /status command to check if the bot is running."""
    uptime = time.time() - start_time
    uptime_str = str(timedelta(seconds=int(uptime)))
    
    status_text = (
        f"{color('Bot Status', 'primary')}\n\n"
        f"Status: {color('Running', 'success')}\n"
        f"Uptime: {uptime_str}\n"
        f"HTTP Server: {color('Active', 'success')}\n"
        f"Port: {PORT}\n\n"
        f"The bot is fully operational and ready to process videos!"
    )
...
class KeepAliveHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b'Screenshot Bot is running! This is a keep-alive server to prevent Render from shutting down the service.')
...
async def keep_alive():
    """Periodically ping the service to keep it alive."""
    while is_running:
        try:
