...
EMOJIS = {
    "start": emoji.WAVING_HAND,
    "help": emoji.OPEN_BOOK,
    "analyzing": emoji.MAGNIFYING_GLASS_TILTED_RIGHT,
    "download": emoji.INBOX_TRAY,
    "processing": emoji.GEAR,
    "generating": emoji.CAMERA_WITH_FLASH,
    "sending": emoji.OUTBOX_TRAY,
    "success": emoji.CHECK_MARK_BUTTON,
    "error": emoji.CROSS_MARK,
    "warning": emoji.WARNING,
    "video": emoji.MOVIE_CAMERA,
    "document": emoji.PAGE_FACING_UP,
    "time": emoji.STOPWATCH,
    "size": emoji.BAR_CHART,
    "duration": emoji.HOURGLASS_NOT_DONE,
    "screenshots": emoji.FRAMED_PICTURE
}
...
async def start_command(client, message):
    ...
    welcome_text = (
        f"{EMOJIS['start']} {color(f'Welcome, {user_name}!', 'primary')}\n\n"
        ...
        f"2. Tell me how many screenshots you want\n"
        f"3. I'll generate beautiful screenshots for you!\n\n"
        f"Type /help to see all available commands."
    )
    
    # Create keyboard with animated buttons
    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(f"{EMOJIS['help']} Help", callback_data="help"),
            InlineKeyboardButton(f"{EMOJIS['video']} Examples", callback_data="examples")
        ]
    ])
...
async def handle_callback(client, callback_query):
    ...
    if data == "help":
        ...
        examples_text = (
            f"{EMOJIS['video']} {color('How Screenshots Work', 'primary')}\n\n"
...
async def help_command(client, message):
    ...
    help_text = (
        ...
        f"3. Tell me how many screenshots you want (1-10)\n"
...
async def about_command(client, message):
    ...
    about_text = (
        ...
        f"• Choose how many screenshots you want\n"
...
async def animated_progress(status_message, action, total_steps, animation_type="spinner"):
    """Display an animated progress indicator."""
    frames = LOADING_ANIMATIONS[animation_type]
    frame_count = len(frames)
    
    for i in range(total_steps):
        frame = frames[i % frame_count]
        try:
            await status_message.edit_text(f"{frame} {action} ({i+1}/{total_steps})")
...
def create_progress_bar(current, total, bar_length=20):
    ...
    # Use different characters for a more visually appealing bar
...
async def handle_file(client, message):
    ...
    status_message = await message.reply_text(f"{EMOJIS['analyzing']} Analyzing file...")
    
    # Show animated dots while analyzing
    analysis_task = asyncio.create_task(
        animated_progress(status_message, "Analyzing file", 5, "dots")
    )
    ...
    try:
        ...
        if message.video:
            ...
            if file_size > MAX_DIRECT_DOWNLOAD_SIZE:
                size_warning = f"\n\n{EMOJIS['warning']} {color('This is a large file. Download may take some time.', 'warning')}"
            
            # Create a visually appealing file details card
            file_details = (
                f"{EMOJIS['video']} {color('File Details', 'primary')}\n\n"
                f"{EMOJIS['document']} {color('Name:', 'secondary')} {file_name}\n"
                f"{EMOJIS['size']} {color('Size:', 'secondary')} {file_size_mb:.2f} MB\n"
                f"{EMOJIS['duration']} {color('Duration:', 'secondary')} {duration_formatted}\n"
                f"{EMOJIS['video']} {color('Type:', 'secondary')} {file_type}{size_warning}\n\n"
                f"{color('How many screenshots would you like?', 'accent')} (Enter a number between 1-10)"
            )
        ...
        # Check if it's a document
            ...
            if is_video:
                # Create a visually appealing file details card
                file_details = (
                    f"{EMOJIS['document']} {color('File Details', 'primary')}\n\n"
                    f"{EMOJIS['document']} {color('Name:', 'secondary')} {file_name}\n"
                    f"{EMOJIS['size']} {color('Size:', 'secondary')} {file_size_mb:.2f} MB\n"
                    f"{EMOJIS['video']} {color('Type:', 'secondary')} {file_type} (Video){size_warning}\n\n"
                    f"{color('How many screenshots would you like?', 'accent')} (Enter a number between 1-10)"
                )
                ...
                error_message = (
                    f"{EMOJIS['error']} {color('Unsupported File', 'error')}\n\n"
                    f"{EMOJIS['document']} {color('Name:', 'secondary')} {file_name}\n"
                    f"{EMOJIS['size']} {color('Size:', 'secondary')} {file_size_mb:.2f} MB\n"
        ...
        error_message = (
            f"{EMOJIS['error']} {color('Error', 'error')}\n\n"
...
async def handle_count_callback(client, callback_query):
    ...
    if count_match and chat_id in user_states and user_states[chat_id].get("waiting_for_count"):
        ...
        await callback_query.answer(f"Generating {count} screenshots...")
        
        # Update the message to show processing
        await callback_query.message.edit_text(
            f"{EMOJIS['processing']} {color('Processing', 'primary')}\n\n"
        ...
        await callback_query.answer("Invalid selection or timed out. Please send a new video.")
        ...
        try:
            ...
            if count < 1 or count > 10:
                await message.reply_text(
                    f"{EMOJIS['warning']} {color('Invalid Number', 'warning')}\n\n"
                    f"Please enter a number between 1 and 10."
                )
                return
            ...
            await message.reply_text(
                f"{EMOJIS['warning']} {color('Invalid Input', 'warning')}\n\n"
                f"Please enter a valid number between 1 and 10."
            )
        except Exception as e:
...
async def progress_callback(current, total, status_message, action_text, emoji_key):
    ...
    try:
        ...
        if total > 0:
            ...
            if progress == 1.0 or int(progress * 20) > int((current - 1024) / total * 20):
                ...
                progress_message = (
                    f"{EMOJIS[emoji_key]} {color(action_text, 'primary')}\n\n"
                    f"{progress_bar}\n"
                    f"{color(f'{current / 1024 / 1024:.1f} MB / {total / 1024 / 1024:.1f} MB', 'secondary')}"
                )
...
async def download_with_retry(client, file_id, file_path, status_message, max_retries=3):
    ...
    while retries < max_retries:
        try:
            ...
            # Create a visually appealing download message
            download_message = (
                f"{EMOJIS['download']} {color(f'Downloading video{retry_text}', 'primary')}\n\n"
                f"{create_progress_bar(0, 100)}\n"
                f"{color('0.0 MB / ? MB', 'secondary')}"
            )
            ...
            async def download_progress(current, total):
                await progress_callback(
                    current, total, status_message, f"Downloading video{retry_text}", "download"
                )
            ...
            if "timed out" in str(e).lower():
                timeout_message = (
                    f"{EMOJIS['warning']} {color('Download Timed Out', 'warning')}\n\n"
                ...
                flood_message = (
                    f"{EMOJIS['warning']} {color('Rate Limit Reached', 'warning')}\n\n"
                ...
                error_message = (
                    f"{EMOJIS['warning']} {color('Download Error', 'warning')}\n\n"
                    f"Error: {str(e)}\n"
                    f"Retrying ({retries}/{max_retries})..."
                )
                await status_message.edit_text(error_message)
...
async def alternative_download(client, chat_id, message_id, file_path, status_message):
    ...
    try:
        alt_download_message = (
            f"{EMOJIS['download']} {color('Alternative Download Method', 'primary')}\n\n"
...
async def generate_screenshots(client, message, chat_id):
    ...
    if not isinstance(message, Message):
        status_message = await client.send_message(
            chat_id,
            f"{EMOJIS['processing']} {color('Preparing', 'primary')}\n\n"
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            screenshot_message = (
                f"{EMOJIS['generating']} {color('Generating Screenshots', 'primary')}\n\n"
                f"{create_progress_bar(0, screenshot_count)}\n"
                f"{color(f'0/{screenshot_count} screenshots', 'secondary')}"
            )
            ...
            for i in range(screenshot_count):
                ...
                timestamp = duration * i / screenshot_count
                ...
                progress_message = (
                    f"{EMOJIS['generating']} {color('Generating Screenshots', 'primary')}\n\n"
                    f"{progress_bar}\n"
                    f"{color(f'{i + 1}/{screenshot_count} screenshots', 'secondary')}"
                )
            ...
            if not screenshot_paths:
                error_message = (
                    f"{EMOJIS['error']} {color('Generation Failed', 'error')}\n\n"
            ...
            sending_message = (
                f"{EMOJIS['sending']} {color('Sending Screenshots', 'primary')}\n\n"
                f"{create_progress_bar(0, len(screenshot_paths))}\n"
                f"{color(f'0/{len(screenshot_paths)} sent', 'secondary')}"
            )
            ...
            for i, path in enumerate(screenshot_paths):
                ...
                timestamp = timedelta(seconds=int(duration * (i / screenshot_count)))
                
                try:
                    # Create a visually appealing caption
                    caption = (
                        f"{EMOJIS['screenshots']} Screenshot {i+1}/{screenshot_count}\n"
                        f"{EMOJIS['time']} Timestamp: {timestamp}"
                    )
                    ...
                    progress_message = (
                        f"{EMOJIS['sending']} {color('Sending Screenshots', 'primary')}\n\n"
                        f"{progress_bar}\n"
                        f"{color(f'{i + 1}/{len(screenshot_paths)} sent', 'secondary')}"
                    )
                    ...
                    error_message = (
                        f"{EMOJIS['warning']} {color('Sending Error', 'warning')}\n\n"
            ...
            # Final message with success animation
            success_message = (
                f"{EMOJIS['success']} {color('Success!', 'success')}\n\n"
                f"Successfully generated and sent {len(screenshot_paths)} screenshots!\n\n"
                f"{EMOJIS['time']} {color('Processing Time:', 'secondary')} {total_time:.1f} seconds\n\n"
